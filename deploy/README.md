...
```bash
ufw allow 8443/tcp
ufw status verbose   # старые правила бота (в т.ч. 80/443) должны остаться на месте
```
Порт 8080 можно не открывать наружу вообще (нужен только для ACME-хука через 80, см. issue-cert.sh) — оставьте закрытым, если не нужен прямой HTTP-доступ.
