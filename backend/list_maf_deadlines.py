...
"3 дня на устранение сломанного МАФ" (озвучено на штабе) — теперь явный
потолок по категории (CATEGORY_SLA_DAYS в app/services/issues.py) поверх
срока по критичности (ISSUE_SLA_DAYS: critical=1, high=3, medium=7,
low=14): берётся минимум из двух, так что критичный МАФ (1 день) остаётся
строже 3 дней, а не критичный (раньше 7/14) — теперь не дольше 3. Раньше
это держалось на допущении, что дефект МАФ в чек-листе всегда
is_critical=TRUE → criticality="high" → 3 дня — но замечание по МАФ,
заведённое вручную с другой критичностью, штабной срок не получало;
явное правило чинит это для новых замечаний. Уже открытые замечания по
МАФ, заведённые до этой правки — см. backfill_maf_due_dates.py (отдельный
скрипт: backfill_issue_due_dates.py про другой случай, NULL due_date,
здесь же он уже есть, просто посчитан по старому правилу). Если штаб
утвердит другую шкалу (называли 1-3-5-7) — отдельная правка, здесь просто
читаются уже проставленные due_date, какая бы шкала их ни считала.
