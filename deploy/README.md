...
⚠️ **Разовое действие после релиза «непривилегированные контейнеры»** (`api`
теперь работает от пользователя `appuser`, UID/GID `10001`, не от root) —
владелец каталога с фото на хосте должен совпадать, иначе загрузка новых
фото начнёт падать с "Permission denied":
```bash
sudo chown -R 10001:10001 /opt/jirajura/backend/uploads
```
Выполнить один раз сразу после первого `docker compose ... build && up -d` с
этим релизом — дальше ничего пересчитывать не нужно, новые файлы будут
создаваться уже с правильным владельцем.
