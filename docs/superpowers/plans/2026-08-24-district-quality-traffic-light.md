...
- [ ] **Step 3: Implement one latest DONE inspection per site**
...
- [ ] **Step 3: Implement the shared palette and table cells**
...
- [ ] **Step 1: Write failing export and period-isolation assertions**
...
- [ ] **Step 3: Implement export fields and state reconstruction**

Render quality columns using the same palette. For the remediation snapshot, use each Issue's last `IssueStatusHistory.new_status` at or before the selected end; without history use its initial `open` state. Do not use today's Issue.status for past periods.
